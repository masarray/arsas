...
def replace_once_in_method(method_start: str, method_end: str, old: str, new: str, label: str) -> None:
    global text
    start = text.find(method_start)
    if start < 0:
        raise SystemExit(f"{label}: method start not found")
    end = text.find(method_end, start)
    if end < 0:
        raise SystemExit(f"{label}: method end not found")
    body = text[start:end]
    count = body.count(old)
    if count != 1:
        raise SystemExit(f"{label}: expected exactly one source match inside method, found {count}")
...
new_setup = """        if (session.Client.CanUseHybridReportPlanner(session.Device))
        ...
        ResetPollQueue(session);
        UpdateDeviceAcquisitionSummary(session);"""
replace_once_in_method(
    "    private async Task TryStartPendingReportSetupAsync(",
    "    private void UpdateDeviceAcquisitionSummary(",
    old_setup,
    new_setup,
    "hybrid-plan-consumption")
