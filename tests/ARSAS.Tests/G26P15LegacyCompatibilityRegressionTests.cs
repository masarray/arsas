...
{
    [Fact]
    public void GuardedRuntime_UsesTypedAriecLegacyCompatibilityPolicyInsteadOfWeakeningDataChangeGate()
    {
        ...
        Assert.Contains("MmsGuardedDynamicReportLegacyCompatibilityPolicy.TryBuildCompatibleContext", guarded, StringComparison.Ordinal);
    ...
    public void LegacyRegistry_IsExactFieldEvidenceNotWildcardAuthorization()
    {
        ...
        Assert.Contains("AA1C1F08R4Q0/CSWI1$ST$Pos$stVal", registry, StringComparison.Ordinal);
        Assert.Contains("AA1C1F08R4Q0/XCBR1$ST$Pos$stVal", registry, StringComparison.Ordinal);
        Assert.Contains("MmsDynamicInformationReportKind.GeneralInterrogation", registry, StringComparison.Ordinal);
        Assert.Contains("ExactSequence(activation.MemberReferences, ExpectedMemberReferences)", registry, StringComparison.Ordinal);
        Assert.Contains("ExactSequence(report.MemberReferences, ExpectedMemberReferences)", registry, StringComparison.Ordinal);
        Assert.Contains("ExactSequence(envelope.ExactProvenMemberReferences, ExpectedMemberReferences)", registry, StringComparison.Ordinal);
    }

    [Fact]
    public void LegacyRegistry_RecordsOnlyTheReviewedNoGiDchgAndCleanupFacts()
    {
        var registry = Read("Services/DynamicReportGuardedLegacyCompatibilityEvidenceRegistry.cs");
