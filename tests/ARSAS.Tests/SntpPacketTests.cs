...
{
    [Fact]
    public void ClientRequest_IsRecognized_AndOriginateTimestampIsEchoed()
    {
    ...
    public void Broadcast_IsMode5_AndUsesLowPriorityLocalStratum()
    {
        ...
        Assert.Equal(4, unchecked((sbyte)packet[2]));
    ...
    [Fact]
    public void UnsynchronizedReply_SetsLeapAlarmAndStratum16()
    {
        var request = new byte[SntpPacket.MinimumLength];
        request[0] = (byte)((4 << 3) | 3);
        SntpPacket.WriteTimestamp(request.AsSpan(40, 8), DateTimeOffset.UtcNow);
