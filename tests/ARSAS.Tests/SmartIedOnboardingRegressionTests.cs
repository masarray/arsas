...
public sealed class SmartIedOnboardingRegressionTests
{
    ...
    [Fact]
    public void Onboarding_OffersSclOrLiveIpDiscoveryWithoutOwningProtocolRuntime()
    {
        var behavior = Read("Services/SmartIedOnboardingBehavior.cs");
        var app = Read("App.xaml.cs");

        Assert.Contains("Open SCL / CID / ICD", behavior, StringComparison.Ordinal);
    ...
    public void Onboarding_ModelEvents_MarshalVisualRefreshThroughMainWindowDispatcher()
    {
        ...
        Assert.DoesNotContain("RefreshConnectAll();", propertyChangedBody, StringComparison.Ordinal);
