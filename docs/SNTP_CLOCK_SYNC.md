...
- Sends an immediate SNTPv4 mode-5 directed broadcast, then repeats at a minimum 16-second interval.
...
- Treats the laptop clock as a low-priority local reference (`stratum 15`, `LOCL`) rather than pretending to be GPS or IEC/IEEE 61850-9-3 PTP.
- Performs a wall-clock sanity/step check. A large time step suppresses one broadcast and causes an unsynchronized unicast response for that instant.
- Never fails an IEC 61850 association when SNTP cannot start.

## UDP/123 ownership

Windows Time and other NTP software may already own UDP/123. ARSAS does **not** stop or reconfigure those services automatically. If bind fails, Clock Sync reports `PortUnavailable` and all MMS/GOOSE/SV behavior continues normally.
...
- leap-alarm/stratum-16 unsynchronized replies;
