...
4. Launch the viewer with `--open <absolute cfg path>` using `ProcessStartInfo.ArgumentList`.
5. Report missing/invalid local packages without changing their transfer state.

ArdIrec responsibilities:

1. Accept `ardirec.exe <record.cfg>` and `ardirec.exe --open <record.cfg>`.
2. Route startup loading through the existing `DocumentController::openCfg()` path.
3. Keep CFG/DAT validation, parsing, waveform loading and analysis inside ArdIrec/`ardirec_core`.
...
2. Pin an exact ArdIrec commit in ARSAS integration metadata.
...
4. Stage the deployed runtime under `Tools/ArdIrec/` in the ARSAS publish directory before Inno Setup runs.
