...
/// this service becomes the assessment authority so the displayed pair and displayed
/// result can never refer to different evidence generations.
/// </summary>
public static class FatCurrentEvidenceAssessmentService
{
    public static FatCurrentEvidenceAssessment Evaluate(IoTestPointPlan point)
    {
        ...
        if (value1.ConnectionGeneration != value2.ConnectionGeneration)
        {
            return new FatCurrentEvidenceAssessment(
                FatAssessmentState.Review,
                "REVIEW: Value 1 and Value 2 belong to different IED connection generations; recapture a coherent current pair.");
        }
    ...
    public static FatCurrentEvidenceAssessment Apply(IoTestPointPlan point)
    {
        ...
        point.Runtime.FatAssessment = assessment.State;
        point.Runtime.FatAssessmentReason = assessment.Reason;
        return assessment;
    }
        ...
        long ConnectionGeneration)
    {
        public static CurrentEvidence From(FatValueEvidence evidence)
            => new(evidence.RawValue, evidence.Quality, evidence.Sequence, evidence.ConnectionGeneration);
