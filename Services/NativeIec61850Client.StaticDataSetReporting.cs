...
public sealed partial class NativeIec61850Client
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        // The model that created the operator-visible DataSet inventory is also the semantic
        // member-order authority for InformationReport mapping. Live directory evidence is
        // verification only; replacing these members with a second raw network projection can
        // re-introduce positional/type ambiguity after discovery has already canonicalized it.
        ...
        foreach (var dataSetGroup in selectedByDataSet)
        {
            ...
            // Smart Discovery already captured GetNamedVariableListAttributes in this same
            // association. Reuse that wire evidence instead of issuing a second directory read,
            // then require exact count/order identity against the canonical model.
            ...
            {
                warnings.Add(
                    $"{dataSetReference}: discovery snapshot has no ordered live DataSet directory. RCB was not armed; re-scan is required and MMS process polling remains disabled.");
                continue;
            }
            ...
            {
                warnings.Add(
                    $"{dataSetReference}: canonical/live DataSet member order mismatch ({directoryMismatch}). RCB was not armed; unsafe positional report projection and MMS process fallback were both refused.");
                continue;
            }
            ...
            var subscription = new ArMms.MmsReportSubscriptionPlan
            {
                ...
                Steps = new[]
                {
                    ...
                    $"Use canonical ordered DataSet members {dataSetReference} ({modelDirectory.Members.Count} members), verified against the discovery snapshot.",
        ...
        CancellationToken cancellationToken)
    {
        ...
        var start = await RunMmsOperationAsync(
            () => _session.StartConfiguredStaticReportMonitorAsync(
                subscription,
