param(
    ...
    [Parameter(Mandatory=$true)][string]$ArsasValidatedHeadCommit,
...
foreach ($entry in @(
    @{ Value = $ArsasValidatedHeadCommit; Label = 'ARSAS validated head commit' },
    @{ Value = $ArsasBaseCommit; Label = 'ARSAS base commit' },
    @{ Value = $EngineHeadCommit; Label = 'engine head commit' },
    @{ Value = $EngineBaseCommit; Label = 'engine base commit' })) { Assert-Commit $entry.Value $entry.Label }

$arsasValidatedHead = $ArsasValidatedHeadCommit.ToLowerInvariant()
...
if ($readiness.Phase -ne 'P0-5g' -or $readiness.Verdict -ne 'READY_FOR_REVIEW') { throw 'P0-5h merge execution requires P0-5g READY_FOR_REVIEW.' }
if (@($readiness.Blockers).Count -ne 0 -or -not [bool]$readiness.ProductionSwitchEnabled) { throw 'P0-5h refuses readiness evidence with blockers or a disabled production switch.' }
if ($promotion.Phase -ne 'P0-5g-authority' -or $promotion.Status -ne 'production-promoted') { throw 'P0-5h requires a production-promoted P0-5g authority.' }
if ($physical.Phase -ne 'P0-5f-authority' -or $physical.Status -ne 'physical-finalized') { throw 'P0-5h requires physical-finalized P0-5f authority.' }
if ($target.Phase -ne 'P0-5h' -or [int]$target.ArsasPullRequest -ne 324 -or [int]$target.EnginePullRequest -ne 134) { throw 'P0-5h target repository/PR authority is invalid.' }
if ([string]$target.MergeMethod -ne 'merge' -or @($target.MergeOrder) -join ',' -ne 'engine,arsas') { throw 'P0-5h target must require merge-commit method and engine-first order.' }
if (([string]$readiness.ArsasHeadCommit).ToLowerInvariant() -ne $arsasValidatedHead -or ([string]$promotion.ArsasValidatedHeadCommit).ToLowerInvariant() -ne $arsasValidatedHead) { throw 'P0-5h ARSAS validated head differs from P0-5g authority.' }
if (([string]$readiness.EngineHeadCommit).ToLowerInvariant() -ne $engineHead -or ([string]$promotion.EngineHeadCommit).ToLowerInvariant() -ne $engineHead) { throw 'P0-5h engine head differs from P0-5g authority.' }
if ([string]$readiness.EngineHeadCiConclusion -ne 'success') { throw 'P0-5h requires green engine-head CI evidence.' }
...
if (([string]$promotion.PhysicalAuthoritySha256).ToLowerInvariant() -ne $physicalHash) { throw 'P0-5g promotion authority is bound to a different physical authority.' }
...
$manifest = [ordered]@{
    ...
    Arsas = [ordered]@{
        ...
        PullRequest = 324
        ValidatedHeadSha = $arsasValidatedHead
        BaseShaAtAuthorization = $arsasBase
        LiveMergeHeadSha = $null
        AllowedPostAuthorizationPaths = @('evidence/smart-discovery-mainline-merge-manifest.json')
    }
    Engine = [ordered]@{
        Repository = 'masarray/ARIEC61850'
        PullRequest = 134
        ExpectedHeadSha = $engineHead
        BaseShaAtAuthorization = $engineBase
    }
    Provenance = [ordered]@{
    ...
    RequiredExecutionChecks = @(
        ...
        'base-sha-unchanged',
        'engine-expected-head-sha-match',
        'arsas-live-head-resolved-at-execution',
        'arsas-post-authorization-diff=merge-manifest-only',
        'merge-method=merge',
        'engine-merge-first',
...
Write-Host "  ARSAS validated head: $arsasValidatedHead"
Write-Host '  ARSAS live merge head: resolve immediately before merge after manifest-only diff check'
